...
function ContactCard({ locale }: { locale: Locale }) {
  const title = getMessage(locale, "seekMedicalHelp.title");
  const description = getMessage(locale, "seekMedicalHelp.description");
  const emergencyHotline = getMessage(
    locale,
    "seekMedicalHelp.emergencyHotline",
  );
  const hotlineNumber = getMessage(locale, "seekMedicalHelp.hotlineNumber");

  return (
    <section
      className="relative mx-auto py-1 px-[32px] lg:px-0 lg:py-4 gap-[10px] md:gap-0 bg-white"
      id="contact"
    >
